...
#define dbagent "/bigdata/rfid/checkpass.php?MAC=%s&KEY=%s"
...
//  app program is ==> /bigdata/rfid/checkpass.php?MAC=112233445566&KEY=0079262864
//  App parameters ==> MAC=112233445566&KEY=0079262864
 
/*
   完整的 HTTP 請求範例，分解如下：
  主機位址：iot.arduino.org.tw:8888（域名與通訊埠）。
  應用程式路徑：/bigdata/rfid/checkpass.php（伺服器上的 PHP 程式，用來接收並處理資料）。
  參數：?MAC=B8D61A68DAFC&KEY=0079262864，表示傳送的資料：
  MAC：設備的唯一識別碼（例如 112233445566）。
  KEY：RFID 卡號:0079262864
 */
//---------------------------------------------------------------------
//----------本程式函式宣告--------------------------------------
...
//  App parameters ==> ?MAC=556885568888&start=20100406&end=20250407
 ...
 if  (Wifi.getStatus())
 {
  ...
  if (String(device) == MacData && String(result) == "Find") 
  {
    ...
    PrintmsgonOLED("Find") ; // show json results
    openDoor() ; //控制第 doorlock 號繼電器，設定為 RelayOpen（開啟）
    delay(2000) ;
    closeDoor() ;  // 控制第 doorlock 號繼電器，設定為 RelayClose（關閉）
  }
  if (String(device) == MacData && String(result) == "notFind") 
  {
    ...
    Serial.print(") is not  Registered and NO PASS \n");
        PrintmsgonOLED("notFind") ; // show json results
  }
 }
}
