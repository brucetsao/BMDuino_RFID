...
String SPACE(int sp);  //sp為傳入產生空白字串長度,生成指定長度的空格字串
String ULongtoString(unsigned long ll) ; // 將一個 unsigned long（無號長整數）轉換成字串（String） 
String genstr(char c,int sp);  //sp為傳入產生空白字串長度,生成指定長度的字元C字串
...
String getjson(String ss) ;  //BMDUINO TCP.h 專用，轉換Wifi.http_getString();//get http result 字串多餘部分
  ...
  if (s1 == -1) //not array
    {
      if (s2 == -1) //not found '{'
        {
          return tmp ;  //return tmpty string
        }
        else
        {
          st1 = s2 ;  //get start point of '{}'
          st2 = ss.lastIndexOf("}") ; // get last start point of '}'
          tmp = ss.substring(st1,st2+1) ; //get json doc
          return tmp ;
        }
    }
    else
