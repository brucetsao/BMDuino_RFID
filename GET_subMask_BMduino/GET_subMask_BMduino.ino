...
void setup() 
{
  ...
  // Serial.println(typeof(Wifi.getGateway())) ; // 註解掉的除錯語句，用於查資料型別
    Serial.println("---Gateway SUB Mask from DHCP----"); // 分隔線，美觀用途
  Serial.println(GetsubMask()); // 印出取得 連接上的SSID熱點之後閘道器IP位址
  // Serial.println(typeof(Wifi.getMask())) ; // 註解掉的除錯語句，用於查資料型別
