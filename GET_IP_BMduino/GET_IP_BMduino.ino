...
void setup() 
{
  ...
  Serial.println(GetSSID()); // 印出 WiFi 模組的連上熱點名稱
  // Serial.println(typeof(Wifi.SSID())) ; // 註解掉的除錯語句，用於查資料型別
  Serial.println("---IP Address from DHCP----"); // 分隔線，美觀用途
  Serial.println(GetIP()); // 印出 WiFi 模組的連上熱點名稱
  // Serial.println(typeof(Wifi.getIP())) ; // 註解掉的除錯語句，用於查資料型別
